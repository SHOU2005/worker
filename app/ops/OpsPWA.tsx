...
export default function OpsPWA() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('ops_pwa_dismissed')) return
  ...
  async function install() {
    ...
    setShow(false)
    localStorage.setItem('ops_pwa_dismissed', '1')
  }
  ...
  return (
    <div style={{
      ...
      animation: 'opwaFadeIn 0.4s ease',
    }}>
      <style>{`
        @keyframes opwaFadeIn { from { opacity: 0 } to { opacity: 1 } }
        @keyframes opwaUp { from { opacity: 0; transform: translateY(24px) } to { opacity: 1; transform: translateY(0) } }
        .opwa-install:hover { background: #F0F0F0 !important; }
        .opwa-later:hover { background: rgba(255,255,255,0.08) !important; }
      `}</style>
      ...
      <div style={{ animation: 'opwaUp 0.5s ease forwards', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: 340 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 40, flexWrap: 'wrap', justifyContent: 'center' }}>
          {['⚡ Instant access', '📴 Works offline', '🔔 Alerts'].map(f => (
            <div key={f} style={{
